...
from exp_ctrl_vqe_cost_function import exp_g1_cost_func_x
...
while not done:
    ...
    for i in range(batch_size):
        ...
        if epoch_type == 'evaluation':
            # run the thing that was supposed to run
            reward_data[i] = exp_g1_cost_func_x(input_pulse_arrs[:, i].flatten(), *client_args)

            # recalibrate iq blobs
            ...
            blobs = measure_progs.single_shot(run=True, debug=False)

            config_dict["qubit"]["g_blob"] = list(blobs[0])
            config_dict["qubit"]["e_blob"] = list(blobs[1])
            config_dict["qubit"]["f_blob"] = list(blobs[2])

            # write updated dict to pulse_configs
            with open("../../utils/pulse_configs.py", "w") as f:
                json.dump(config_dict, f)

            # end recalibrate I and Q blobs
