...
with open(sf_name, "rb") as fp:
    epochs, train_batch_size, init_amps, init_freqs, amp_scales, freq_scales, hdf5_name = pickle.load(fp)
fp.close()
epochs = int(epochs)
...
n_array_vals = np.shape(init_amps)[1] # number of array values to optimize
...
action_spec = {f'pulse_array_{i}': specs.TensorSpec(shape=[n_array_vals], dtype=tf.float32)
