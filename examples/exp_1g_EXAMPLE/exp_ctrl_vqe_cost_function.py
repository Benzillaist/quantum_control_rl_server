...
def exp_g1_cost_func_x(opts, *args):
    ...
    observable_dicts = [
        ...
        [
            pc.pi_pulse(rabi_states="ef", meas_state=measure_state),
            pc.pi_pulse(rabi_states="ge", meas_state=measure_state, gain_mult=0, selective=True),
        ],
    ]
    ...
    if verbose:
        print(f'Measurement 0: i: avg: {np.average(i_arr_m0)}, std:{np.std(i_arr_m0)}, q: {np.average(q_arr_m0)}, std:{np.std(q_arr_m0)}')
        print(f'Measurement 1: i: avg: {np.average(i_arr_m1)}, std:{np.std(i_arr_m1)}, q: {np.average(q_arr_m1)}, std:{np.std(q_arr_m1)}')
    ...
    reward = avg_mapped_probs_m1 - avg_mapped_probs_m0
