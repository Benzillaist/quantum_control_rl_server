...
(host, port) = '127.0.0.1', 5553 # ip address of RL server, here it's hosted locally
...
while not done:
    ...
    for i in range(batch_size):
        ...
        if epoch_type == 'evaluation':
            reward_data[i] = exp_g1_cost_func_xt(np.append(np.append(input_pulse_arrs[:, i].flatten(), input_time_arrs[:, i].flatten()), input_freq_arrs[:, i]), *client_args)
