...
with open(sf_name, "rb") as fp:
    epochs, train_batch_size, init_amps, init_times, init_freqs, amp_scales, time_scales, freq_scales, hdf5_name = pickle.load(fp)
fp.close()
epochs = int(epochs)
train_batch_size = int(train_batch_size)
init_amps = np.array(init_amps).astype(np.float32)
init_times = np.array(init_times).astype(np.float32)
init_freqs = np.array(init_freqs).astype(np.float32)
amp_scales = np.array(amp_scales).astype(np.float32)
time_scales = np.array(time_scales).astype(np.float32)
freq_scales = np.array(freq_scales).astype(np.float32)
...
root_dir = r'/examples/old/ctrl_vqe_pi_pulse_sim_entangle_interp2/save_data'  # root_dir = os.getcwd() #r'E:\rl_data\exp_training\pi_pulse'
...
n_time_vals = np.shape(init_times)[1] # number of array values to optimize
...
# Adds times
if len(init_times[0]) == 1:
    ascript_temp = {f'time_array_{i}': [time_seg] for i, time_seg in enumerate(init_times)}
    action_script.update(ascript_temp)
else:
    ascript_temp = {f'time_array_{i}': [list(time_seg)] for i, time_seg in enumerate(init_times)}
    action_script.update(ascript_temp)
# Adds frequencies
ascript_temp = {f'freq_{i}': [[freq]] for i, freq in enumerate(init_freqs)}
action_script.update(ascript_temp)
...
# Adds times
aspec_temp = {f'time_array_{i}': specs.TensorSpec(shape=[n_time_vals], dtype=tf.float32)
               for i in range(2 * n_drives)}
action_spec.update(aspec_temp)
# Adds frequencies
aspec_temp = {f'freq_{i}': specs.TensorSpec(shape=[1], dtype=tf.float32) for i in range(n_drives)}
action_spec.update(aspec_temp)
...
# Adds times
ascale_temp = {f'time_array_{i}': time_scale for i, time_scale in enumerate(time_scales)}
action_scale.update(ascale_temp)
# Adds frequencies
ascale_temp = {f'freq_{i}': freq_scale for i, freq_scale in enumerate(freq_scales)}
action_scale.update(ascale_temp)
