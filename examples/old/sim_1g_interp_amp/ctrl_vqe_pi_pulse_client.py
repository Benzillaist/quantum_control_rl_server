...
from utils.opt_utils import sim_interp_cost_eval_no_time
...
def cost_qAB_g11(final_expect, final_dm):
    return np.power(np.abs(final_dm.full()[9][0]), 2)
...
(host, port) = '127.0.0.1', 5565 # ip address of RL server, here it's hosted locally
...
while not done:
    ...
    for i in range(batch_size):
        ...
        if epoch_type == 'evaluation':
            reward_data[i] = sim_interp_cost_eval_no_time(input_pulse_arrs[:, i].flatten(), *client_args)
