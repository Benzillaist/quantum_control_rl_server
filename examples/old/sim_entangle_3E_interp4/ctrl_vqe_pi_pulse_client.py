...
from utils.opt_utils import sim_interp_cost_eval
...
def cost_dihydrogen_entangle(final_expect, final_state):
    ...
    print(f'p_10: {p_10}')
    print(f'p_11: {p_11}')
    print(f'c_0002: {c_0002}')
    print(f'c_2001: {c_2001}')

    return (np.power(np.abs(final_state[0][0]), 2) +
            np.power(np.abs(final_state[2][0]), 2) +
            np.power(np.abs(final_state[1][0]), 2) +
            np.power(np.abs(final_state[(2 * cavity_dims) - 1][0]), 2) +
            np.power(np.abs(final_state[cavity_dims - 1][0]), 2) +
            np.power(np.abs(final_state[cavity_dims]), 2) +
            np.abs(final_state[0][0] * final_state[2][0]) +
            np.abs(final_state[(2 * cavity_dims) - 1][0] * final_state[1][0]))
...
(host, port) = '127.0.0.1', 5568 # ip address of RL server, here it's hosted locally
...
while not done:
    ...
    for i in range(batch_size):
        ...
        if epoch_type == 'evaluation':
            reward_data[i] = sim_interp_cost_eval(np.append(input_pulse_arrs[:, i].flatten(), input_time_arrs[:, i].flatten()), *client_args)
