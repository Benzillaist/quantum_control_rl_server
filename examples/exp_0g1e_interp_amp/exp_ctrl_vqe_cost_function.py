...
def exp_g1_cost_func_x(opts, *args):
    ...
    expt_dict = {
        "reps": 1,  # inner repeat times
        "rounds": 100,  # outer loop times
    ...
    config["readout"]["relax_delay"] = 2000
        ...
        [
            ...
            pc.pi_pulse(rabi_states="ge", meas_state=measure_state, selective=True,
                        freq=config_dict["qubit"]["selective_freqs"]["g"]["ge"] + config_dict["storage_A"][
                            "chi"]),
            pc.pi_pulse(rabi_states="ge", meas_state=measure_state),
        ],
    ]
    ...
    if plot_pulse_viewer:
        for i in range(len(observable_dicts)):
            normVolt.pulse_view(title="0g1e exp", n=i)

    [[g0_i_arr, e1_i_arr], [g0_q_arr, e1_q_arr]] = np.array(normVolt.measure(progress=True, save=False, all_shots=True))[:, :, 0, :]

    g0_mapped_prob = 1 - np.average(SSR_map(g0_i_arr, g0_q_arr, config["qubit"]["g_blob"], config["qubit"]["e_blob"]))
    e1_mapped_prob = 1 - np.average(SSR_map(e1_i_arr, e1_q_arr, config["qubit"]["g_blob"], config["qubit"]["e_blob"]))

    print(f'g0_mapped_prob: {g0_mapped_prob}, e1_mapped_prob: {e1_mapped_prob}')

    cost = 0.5 * np.power(np.average(g0_mapped_prob) + np.averages(e1_mapped_prob))

    # cost = round(np.abs(q_amp / contrast))  # + np.abs(np.sum(np.sqrt(np.power(i_amps, 2) + np.power(q_amps, 2))) / 100000000)
    if verbose:
        print(f'Previous cost: {cost}')
...
#     if verbose:
#         for i in range(num_drives):
#             print(f'Trial I amp {i}: {times[2 * i]}')
#             print(f'Trial I amp {i}: {amps[2 * i]}')
#             print(f'Trial Q amp {i}: {times[(2 * i) + 1]}')
#             print(f'Trial Q amp {i}: {amps[(2 * i) + 1]}')
...
#     config["eval"]["measure_state"] = measure_state
#
#     config["readout"]["relax_delay"] = 800
#
#     config["res"]["freq"] = config["res"]["freqs"][measure_state]
#
#     observable_dicts = [
